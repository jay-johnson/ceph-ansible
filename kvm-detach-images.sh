...
for node in $nodes; do
    ...
    if [[ ! -e ${image_path} ]]; then
        ...
        if [[ "${node}" == "c1b" ]] || [[ "${node}" == "c2b" ]] || [[ "${node}" == "c3b" ]]; then
            ...
            anmt "detaching image: ${image_path} to ${name_of_vm} with:"
            anmt "virsh detach-disk ${name_of_vm} \
                ${image_path} \
                --persistent"
            virsh detach-disk ${name_of_vm} \
                ${image_path} \
                --persistent
        else
            anmt "detaching image: ${image_path} to ${node} with:"
            anmt "virsh detach-disk ${node} \
                ${image_path} \
                --persistent"
